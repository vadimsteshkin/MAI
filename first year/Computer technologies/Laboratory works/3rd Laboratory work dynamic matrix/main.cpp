#include <iostream>
#include <fstream>

const int N_MAX = 15; // Максимальный размер матрицы
const char* FNAMEA = "test.txt"; // Матрица A
const char* FNAMEB = "testb.txt"; // Матрица B

// Чтение матрицы из файла
int InputData(const char* FNAME, double**& Matr, int& rows, int& columns)
{
    setlocale(LC_ALL, "Russia");
    std::ifstream file(FNAME);
    if (!file)
    {
        return 1; // Файл не найден
    }
    file >> rows >> columns;
    if (file.fail())
    {
        file.close();
        return 2; // Размер не число
    }
    if ((rows > N_MAX) || (columns > N_MAX))
    {
        file.close();
        return 3; // Размер больше заданного максимума
    }

    Matr = new double* [rows];
    for (int i = 0; i < rows; i++)
    {
        Matr[i] = new double[columns];
        for (int j = 0; j < columns; j++)
        {
            file >> *(*(Matr + i) + j);
            if (file.fail())
            {
                file.close();
                return 4; // В матрице есть символ, отличный от числа
            }
            if ((file.eof()) and (i < rows - 1 or j < columns - 1))
            {
                file.close();
                return 5;
            }
        }
    }
    if (!(file.eof()))
    {
        file.close();
        return 6; // Элементов больше, чем заданный размер
    }
    file.close();
    return 0;
}

// Печать ошибок
void PrintErrorMessage(int errorCode)
{
    switch (errorCode)
    {
    case 1:
        std::cout << "Файл не найден" << std::endl;
        break;
    case 2:
        std::cout << "Размер не число" << std::endl;
        break;
    case 3:
        std::cout << "Размер больше заданного максимума" << std::endl;
        break;
    case 4:
        std::cout << "В матрице есть символ, отличный от числа" << std::endl;
        break;
    case 5:
        std::cout << "Элементов меньше, чем заданный размер";
        break;
    case 6:
        std::cout << "Элементов больше, чем заданный размер" << std::endl;
        break;
    default:
        std::cout << "Неизвестная ошибка" << std::endl;
        break;
    }
}

// Печать матрицы
void PrintMatrix(double** Matr, int rows, int columns)
{
    std::cout << rows << " * " << columns << std::endl;
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < columns; j++)
        {
            std::cout << " " << *(*(Matr+i)+j);
        }
        std::cout << std::endl;
    }
}

// Нахождение суммы элементов строки
double GetRowSum(double** Matr, int row, int columns)
{
    double sum = 0;
    for (int j = 0; j < columns; j++)
    {
        sum += *(*(Matr + row) + j);
    }
    return sum;
}

// Нахождение индекса строки с минимальной суммой элементов
int FindMinSumRow(double** Matr, int rows, int columns)
{
    int minRow = 0;
    double minSum = GetRowSum(Matr, 0, columns);
    for (int i = 1; i < rows; i++)
    {
        double sum = GetRowSum(Matr, i, columns);
        if (sum < minSum)
        {
            minSum = sum;
            minRow = i;
        }
    }
    return minRow;
}

// Нахождение индекса строки с максимальной суммой элементов
int FindMaxSumRow(double** Matr, int rows, int columns)
{
    int maxRow = 0;
    double maxSum = GetRowSum(Matr, 0, columns);
    for (int i = 1; i < rows; i++)
    {
        double sum = GetRowSum(Matr, i, columns);
        if (sum > maxSum)
        {
            maxSum = sum;
            maxRow = i;
        }
    }
    return maxRow;
}
void DeleteMatr(double** Matr, int rows)
{
    for (int i = 0; i < rows; i++) {
        delete[] *(Matr + i);
    }
    delete[] Matr;
}
// Обмен строк
void SwapRows(double** Matr, int row1, int row2, int columns)
{
    double* temp = Matr[row1];
    Matr[row1] = Matr[row2];
    Matr[row2] = temp;
}

int main()
{
    setlocale(LC_ALL, "RU");
    int columnsA = 0;
    int rowsA = 0; // Размер матрицы A
    double** MatrA = nullptr; // Матрица A

    int columnsB = 0;
    int rowsB = 0; // Размер матрицы B
    double** MatrB = nullptr; // Матрица B

    int errorCodeA = InputData(FNAMEA, MatrA, rowsA, columnsA);
    if (errorCodeA != 0)
    {
        PrintErrorMessage(errorCodeA);
        return 0;
    }

    std::cout << "Матрица A:" << std::endl;
    PrintMatrix(MatrA, rowsA, columnsA);
    int minSumRowA = FindMinSumRow(MatrA, rowsA, columnsA);
    int maxSumRowA = FindMaxSumRow(MatrA, rowsA, columnsA);

    SwapRows(MatrA, minSumRowA, maxSumRowA, columnsA);

    std::cout << "Матрица A после обмена строк:" << std::endl;
    PrintMatrix(MatrA, rowsA, columnsA);
    DeleteMatr(MatrA, rowsA);
    int errorCodeB = InputData(FNAMEB, MatrB, rowsB, columnsB);
    if (errorCodeB != 0)
    {
        PrintErrorMessage(errorCodeB);
        return 0;
    }

    std::cout << "Матрица B:" << std::endl;
    PrintMatrix(MatrB, rowsB, columnsB);

    int minSumRowB = FindMinSumRow(MatrB, rowsB, columnsB);
    int maxSumRowB = FindMaxSumRow(MatrB, rowsB, columnsB);

    SwapRows(MatrB, minSumRowB, maxSumRowB, columnsB);

    std::cout << "Матрица B после обмена строк:" << std::endl;
    PrintMatrix(MatrB, rowsB, columnsB);
    DeleteMatr(MatrB, rowsB);
     return 0;
}